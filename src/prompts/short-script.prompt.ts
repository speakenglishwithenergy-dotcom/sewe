...
export function buildShortScriptPrompt(
  ...
  const duration = shortDurationTargets();
  ...
  const exampleBeats = buildExampleBeats(hookSpeaker, bodySpeaker, short.minLines);
...
- Target length: ${duration.minSeconds}–${duration.maxSeconds} seconds (~${duration.minWords}–${duration.maxWords} words total across ALL beats)
...
REVISION CHECKLIST:
1. LENGTH — ~${duration.minWords}–${duration.maxWords} words total, ${short.minLines}–${short.maxLines} beats (~${duration.minSeconds}–${duration.maxSeconds}s spoken). Do not pad past ~${duration.maxWords} words.
...
function buildExampleBeats(hookSpeaker: string, bodySpeaker: string, minLines: number): string {
  const lines: string[] = [
    `    { "speaker": "${hookSpeaker}", "text": "One short punchy hook — 6–14 words, names the topic and sparks curiosity" }`,
  ];
  const teachingCount = Math.max(1, minLines - 2);
  for (let i = 0; i < teachingCount; i++) {
    lines.push(
      `    { "speaker": "${bodySpeaker}", "text": "Teaching beat ${i + 1} — sharp insight or concrete example" }`,
    );
  }
  lines.push(
